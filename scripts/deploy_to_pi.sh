...
ssh "${SSH_ARGS[@]}" -t "$PI_TARGET" "cd '$REMOTE_SRC' && sudo env INSTALL_AI='$INSTALL_AI' INSTALL_BIRDNET='$INSTALL_BIRDNET' INSTALL_SPECIESNET='$INSTALL_SPECIESNET' INSTALL_CAMERA='$INSTALL_CAMERA' DISABLE_BLUETOOTH_UART='$DISABLE_BLUETOOTH_UART' CONFIG_TEMPLATE='$CONFIG_TEMPLATE' AUDIO_DEVICE='$AUDIO_DEVICE' SERVICE_USER='$PI_USER' scripts/install_pi.sh"
...
if [[ "$REBOOT_AFTER_INSTALL" == "1" ]]; then
  ssh "${SSH_ARGS[@]}" -t "$PI_TARGET" "sudo reboot"
else
