...
class BirdNetRunner:
    ...
    def _species_list_path(self) -> str | None:
        mode = (self.config.species_filter_mode or "generated_list").strip().lower()
        if mode in {"generated_list", "generated", "custom_list", "custom"}:
            return self.config.species_list_path
        if mode in {"birdnet_location", "birdnet", "native_location", "native", "location"}:
            return None
        LOGGER.warning(
